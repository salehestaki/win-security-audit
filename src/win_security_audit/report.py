...
def render_html(report: AuditReport) -> str:
    ...
    sections_index = "\n".join(render_section_link(section) for section in report.sections)
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .section-actions {{ display: inline-flex; gap: 8px; align-items: center; flex-wrap: wrap; justify-content: flex-end; }}
    ...
    .section-link strong {{ font-size: 14px; }}
    .section-link span {{ color: var(--muted); font-size: 12px; }}
    .toolbar {{ display: flex; gap: 8px; flex-wrap: wrap; margin: 2px 0 14px; }}
    .toolbar button {{
      border: 1px solid var(--line);
      background: #fff;
      border-radius: 6px;
  ...
  <script>
    function setSections(mode) {{
      document.querySelectorAll('details.section').forEach(function(section) {{
        if (mode === 'flagged') {{
          section.open = section.dataset.status === 'review' || section.dataset.status === 'suspicious';
        }} else {{
          section.open = mode === 'open';
        }}
      }});
    }}
...
<body>
  ...
  <main>
    ...
    <nav class="section-grid" aria-label="Report sections">
      {sections_index}
    </nav>
    <div class="toolbar">
      <button type="button" onclick="setSections('flagged')">Open flagged</button>
      <button type="button" onclick="setSections('open')">Open all</button>
      <button type="button" onclick="setSections('closed')">Close all</button>
    </div>
    {sections_html}
    <footer>
...
def render_section(section: Any) -> str:
    ...
    return f"""
    <details class="section" id="{esc(section.key)}" data-status="{esc(section.status)}">
      <summary>
        ...
        <div class="section-actions">
          <span class="chip info">{finding_count} findings</span>
          <span class="chip info">{table_count} tables</span>
...
def render_section_link(section: Any) -> str:
    return f"""
      <a class="section-link" href="#{esc(section.key)}">
        <strong>{esc(section.title)}</strong>
        <span>{STATUS_META.get(section.status, STATUS_META[Status.INFO])["icon"]} {esc(STATUS_META.get(section.status, STATUS_META[Status.INFO])["label"])} · {len(section.findings)} findings</span>
      </a>
"""
